...
def tracer_complex_gaussians(
    ...
    return _TracerComplexGaussians.apply(
        means_3d,
        cov3d_precomp,
        signal_precomp,
        attenuation,
...
class _TracerComplexGaussians(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, means_3d, cov3d_precomp, signal_precomp, attenuation, raster_settings
    ):
        ...
        # Note: gaus_radii removed - now computed inside CUDA kernel
        args = (
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, color, geomBuffer, binningBuffer, imgBuffer = (
                    _C.tracer_complex_gaussians(*args)
                )
        ...
        if raster_settings.debug:
            ...
            try:
                (
                    ...
                    grad_signal_precomp,
                    grad_attenuation,
                ) = _C.tracer_complex_gaussians_backward(*args)
...
class ComplexGaussianTracer(nn.Module):
    ...
    def forward(self, means_3d, cov3d_precomp, signal_precomp, attenuation):
        ...
        return tracer_complex_gaussians(
