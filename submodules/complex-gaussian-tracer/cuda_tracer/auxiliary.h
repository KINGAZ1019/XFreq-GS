...
// cov3d layout: [a00, a01, a02, a11, a12, a22] (upper triangular, symmetric)
// Returns the maximum eigenvalue (spatial variance)
__forceinline__ __device__ float compute_cov3d_max_eigenvalue(const float* cov3d)
{
    float a00 = cov3d[0], a01 = cov3d[1], a02 = cov3d[2];
    float a11 = cov3d[3], a12 = cov3d[4], a22 = cov3d[5];

    // Coefficients of characteristic polynomial: det(A - λI) = -λ³ + c2*λ² - c1*λ + c0
    float c2 = a00 + a11 + a22;  // trace
    float c1 = a00*a11 + a00*a22 + a11*a22 - a01*a01 - a02*a02 - a12*a12;
    float c0 = a00*a11*a22 + 2.0f*a01*a02*a12 - a00*a12*a12 - a11*a02*a02 - a22*a01*a01;  // determinant

    // Normalize: λ³ - c2*λ² + c1*λ - c0 = 0
    // Substitution: λ = t + c2/3 to get depressed cubic: t³ + p*t + q = 0
    float c2_3 = c2 / 3.0f;
    float p = c1 - c2 * c2_3;
    float q = c0 - c1 * c2_3 + 2.0f * c2_3 * c2_3 * c2_3;

    // For real symmetric matrix, discriminant >= 0, use trigonometric solution
    float p_3 = p / 3.0f;
    float q_2 = q / 2.0f;
    ...
    if (discriminant <= 0.0f) {
        ...
        float r = sqrtf(fmaxf(-p_3 * p_3 * p_3, 0.0f));
        float r_cbrt = cbrtf(fmaxf(r, 1e-12f));
        float cos_arg = fminf(fmaxf(-q_2 / fmaxf(r, 1e-12f), -1.0f), 1.0f);
        float theta = acosf(cos_arg) / 3.0f;

        // Three roots: 2*r^(1/3)*cos(θ), 2*r^(1/3)*cos(θ + 2π/3), 2*r^(1/3)*cos(θ + 4π/3)
        float root1 = 2.0f * r_cbrt * cosf(theta) + c2_3;
        float root2 = 2.0f * r_cbrt * cosf(theta + 2.0943951f) + c2_3;  // 2π/3
        float root3 = 2.0f * r_cbrt * cosf(theta + 4.1887902f) + c2_3;  // 4π/3
        
        max_eigenvalue = fmaxf(fmaxf(root1, root2), root3);
    } else {
        // One real root (edge case, shouldn't happen for valid covariance)
        float sqrt_disc = sqrtf(discriminant);
        float u = cbrtf(-q_2 + sqrt_disc);
        float v = cbrtf(-q_2 - sqrt_disc);
        max_eigenvalue = u + v + c2_3;
    }

    return fmaxf(max_eigenvalue, 0.0f);  // Eigenvalue must be non-negative
}


// Compute spatial radius and convert to angle for tile coverage
// Returns angle in degrees, safe from NaN
__forceinline__ __device__ float compute_gaussian_angle_radius(
    const float* cov3d,
    float radius_rx,
    float radii_scale = 3.0f)
{
    float max_eigenvalue = compute_cov3d_max_eigenvalue(cov3d);
    float spatial_radius = radii_scale * sqrtf(fmaxf(max_eigenvalue, 0.0f));
    
    // Safe angle calculation: theta = 2 * arcsin(r / (2*R))
    float ratio = spatial_radius / (2.0f * fmaxf(radius_rx, 1e-6f));
    ratio = fminf(fmaxf(ratio, -1.0f), 1.0f);  // Clamp to [-1, 1] for asin safety
    
    float angle_rad = 2.0f * asinf(ratio);
    float angle_deg = angle_rad * (180.0f / PI);
    
    return fmaxf(angle_deg, 1.0f);  // Minimum angle to avoid zero-area tiles
}
