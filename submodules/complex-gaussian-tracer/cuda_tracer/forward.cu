							   ...
							   )
{
	...
	// Compute angle radius directly from covariance matrix
	const float RADII_SCALE = 8.0f;
	float angle_radius = compute_gaussian_angle_radius(cov3d, radius_rx, RADII_SCALE);
	...
	if (use_aos) {
		getRect_v2(p_orig, cov3d, spectrum_3d_coarse, *rx_pos, tile_grid, rect_min, rect_max);
	} else {
